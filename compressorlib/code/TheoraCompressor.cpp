...
extern unsigned int g_imageWidthPixels;
...
TheoraCompressor::TheoraCompressor()
{
	...
	m_info.frame_width = g_imageWidthPixels;
	m_info.frame_height = g_imageHeightPixels;
	m_info.pic_width = g_imageWidthPixels;
	m_info.pic_height = g_imageHeightPixels;
	m_info.pic_x = 0 ;
	m_info.pic_y = 0;
...
unsigned char* TheoraCompressor::compressFrame(const unsigned char* uncompressedFrame, int frameWidth, int frameHeight)
{
	...
	m_currentFrameBuffer = bitstream;
	return bitstream;
}
